...
class BeautifulSoupHTMLParser(HTMLParser, DetectsXMLParsedAsHTML):
    ...
    def handle_starttag(
        ...
        # --- BEGIN SoupReplacer hook (Milestone 3) ---
        if hasattr(self.soup, "replacer") and self.soup.replacer and tag:
            # Simple M2 rename
            if hasattr(self.soup.replacer, "replace_tag"):
                tag.name = self.soup.replacer.replace_tag(tag.name)
            # Extended M3 transformations
            if hasattr(self.soup.replacer, "transform_tag"):
                self.soup.replacer.transform_tag(tag)
