...
# SoupReplacer: a simple tag-replacement filter, inspired by SoupStrainer

class SoupReplacer:
    """
    Extended tag replacer (Milestone 3).

    Supports:
      - Simple tag replacement: SoupReplacer("b", "blockquote")
      - Functional transformations:
          * name_xformer(tag): returns new name
          * attrs_xformer(tag): returns new attrs dict
          * xformer(tag): performs side effects on tag
    """

    def __init__(self, og_tag=None, alt_tag=None,
                 name_xformer=None, attrs_xformer=None, xformer=None):
        # Backward compatibility (Milestone 2)
        self.og_tag = og_tag
        self.alt_tag = alt_tag

        # New Milestone 3 transformers
        self.name_xformer = name_xformer
        self.attrs_xformer = attrs_xformer
        self.xformer = xformer

    def replace_tag(self, name):
        """
        Backward-compatible helper for Milestone 2 simple replacement.
        Only used if parsing logic calls it directly.
        """
    ...
    def transform_tag(self, tag):
        """
        Perform transformations on a tag (Milestone 3 core logic).
        Called during parsing for each tag node.
        """
        # --- Milestone 2 support ---
        ...
        # --- Full transformer (can mutate directly) ---
        if self.xformer:
            self.xformer(tag)
