...
class SoupReplacer:
    """
    Simple tag replacer similar to SoupStrainer.
    Replaces all occurrences of og_tag with alt_tag during parsing.
    """
    ...
    def replace_tag(self, name):
        """Return replacement name if tag matches og_tag."""
        if name == self.og_tag:
            return self.alt_tag
        return name
